...
export class DatabaseService {
  ...
  getPublishedPosts(): Query<DocumentData> {
    return this.database.collection('posts').ref.where('isPublished', '==', true);
  }
  ...
  // TODO: Remove this
  getBourbon() {
    return this.database.collection('whiskey').ref.where('type', '==', 'bourbon');
  }

  // TODO: Remove this
  getOldBourbon() {
    return this.getBourbon().where('age', '>=', 10);
  }

  getKentuckyWhiskey() {
    return this.database.collectionGroup('distillery', ref => ref.where('location', '==', 'kentucky'));
  }
}
